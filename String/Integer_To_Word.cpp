...
string numberToWords(int num)
{
    ...
    for (auto it : mp)
    {
        if (num >= it.first)            // finding the number from map that is just less than num
        {
            string a = "";              // num= 12756 can be divied into three parts a=12  b=thousands c=756 then recursion again
            if (num >= 100)
            {
